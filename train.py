...
import tasks
import datautils
from misc import init_dl_program, name_with_datetime, pkl_save, data_dropout
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="The batch size (defaults to 8)",
    )
    parser.add_argument(
        "--lr",
        type=float,
    ...
    parser.add_argument(
        "--epochs", type=int, default=None, help="The number of epochs"
    )
    parser.add_argument(
        "--save-every",
        type=int,
    ...
    device = init_dl_program(
        args.gpu, seed=args.seed, max_threads=args.max_threads
    )

    print("Loading data... ", end="")
    if args.loader == "UCR":
        task_type = "classification"
        train_data, train_labels, test_data, test_labels = datautils.load_UCR(
            args.dataset
        )

    elif args.loader == "UEA":
        task_type = "classification"
        train_data, train_labels, test_data, test_labels = datautils.load_UEA(
            args.dataset
        )
        ...
        (
            ...
            pred_lens,
            n_covariate_cols,
        ) = datautils.load_forecast_npy(args.dataset, univar=True)
        train_data = data[:, train_slice]
        ...
        (
            ...
            all_test_timestamps,
            delay,
        ) = datautils.load_anomaly(args.dataset)
    ...
    if args.irregular > 0:
        if task_type == "classification":
            ...
            raise ValueError(
                f"Task type {task_type} is not supported when irregular>0."
            )
    print("done")
    ...
    run_dir = (
        "training/" + args.dataset + "__" + name_with_datetime(args.run_name)
    )
